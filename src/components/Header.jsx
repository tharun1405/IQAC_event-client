...
const Header = () => {
  ...
  const handleRole = () => {
    ...
    const targetRoute = location.pathname === '/' ? '/master' : '/';
    navigate( targetRoute );   // Navigate to the target route
  };

  return (
    <header className='flex justify-between w-full items-center bg-gradient-to-r from-black to-black py-2 md:px-8 px-4 shadow-black/15 shadow-md drop-shadow'>

     {/*} <Link to='/' className='text-white font-bold sm:text-2xl text-xl hover:opacity-85 active:opacity-70'>
       
      </Link>
      <Link
        to={ location.pathname === '/' ? '/master' : '/' }
